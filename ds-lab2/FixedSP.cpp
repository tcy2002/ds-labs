...
vector<int> FixedSP::getFixedPointShortestPath(int source, vector<int> intermediates)
{
    ...
    //计算源节点到各必经点的最短路径与最短路径长度
    for (i = 0; i < len; i++)
        pool.push(&FixedSP::getShortestDistance, this,
                  &path_to_source[i],
    ...
    pool.stop(); //等待所有任务完成并暂停线程池

    for (i = 0; i < len; i++) {
        if (distance_to_source[i] > INF - 1) return path;
        reverse(path_to_source[i].begin(), path_to_source[i].end());
    }

    for (i = 0; i < len - 1; i++) {
        for (j = i + 1; j < len; j++) {
            if ((distances[j][i] = distances[i][j]) > INF - 1) return path;
            paths[j][i] = paths[i][j];
            reverse(paths[i][j].begin(), paths[i][j].end());
        }
    }
    QueryPerformanceCounter(&end_time);
    time_tmp = (double)(end_time.QuadPart - begin_time.QuadPart) * 1000.0 / (double)freq_.QuadPart;
    total1 += time_tmp;
    cout << "num: " << intermediates.size() << " time: " << time_tmp << "ms" << endl;
    ...
    time_tmp = (double)(end_time.QuadPart - begin_time.QuadPart) * 1000.0 / (double)freq_.QuadPart;
    total2 += time_tmp;
//    cout << "num: " << intermediates.size() << "ms" << endl;

    return path;
}

void FixedSP::getShortestDistance(vector<int> *path, int *path_len, int det, int ter) {
    if (det == ter) {
        *path_len = 0;
        return;
    }
